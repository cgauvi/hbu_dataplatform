...
The neighborhood axis is dynamic - see `urban_rag.partitions` - so the set of
boroughs the pipeline scrapes lives in Dagster's instance storage rather than
in code. This is the command that edits it::

    python -m urban_rag.dagster_home python -m urban_rag.neighborhoods list
    python -m urban_rag.dagster_home python -m urban_rag.neighborhoods add CIL
    python -m urban_rag.dagster_home python -m urban_rag.neighborhoods remove CIL

Run through `urban_rag.dagster_home` (or `make neighborhood-add`) so the
