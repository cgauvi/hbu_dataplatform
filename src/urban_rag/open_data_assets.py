...
#: fourth city needs a bounding box and no code at all. See `urban_rag.rqtt`.
...
#: `urban_rag.partitions.QUEBEC_BOROUGH_ABBREVIATIONS` are. Written beside
...
#: Québec portal Quebec City's layers come from - see `urban_rag.saguenay` for
...
class StreetNetworkConfig(Config):
    ...
    because the RQTT publishes no municipality code - see `urban_rag.rqtt`.
...
def borough_boundary(store: ParquetStore, scrape_date: str, neighborhood: str):
    ...
    `urban_rag.bdoi_assets`. A Quebec City key is cut out of the
...
def city_bounds(
    ...
    silver against the real boundary - see `urban_rag.street_assets`.
    """
