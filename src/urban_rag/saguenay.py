...
`urban_rag.zoning_grid` reads it. Quebec City publishes its zones as an ArcGIS
layer and its norms as one workbook for the city, which is what
`urban_rag.quebec` exists for. Saguenay splits the difference:
...
its own URL, Saguenay goes through `urban_rag.rag_assets.linked_documents` and
...
#: no Saguenay-specific street feed to name. See `urban_rag.rqtt`.
...
def _families(codes: Sequence[str]) -> tuple[tuple[str, ...], dict[str, str]]:
    ...
    reason `urban_rag.quebec.grid_columns` emits them: Saguenay's classes do
