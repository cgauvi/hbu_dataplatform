...
Free of Dagster and of psycopg the way `tile_grid` is. `urban_rag.tile_render`
turns these specs into the ``ST_AsMVT`` statements that produce the tiles,
`urban_rag.pmtiles_archive` packs a layer's tiles into one PMTiles file, and
`urban_rag.tile_assets` runs both per partition and writes the result into the
...
from urban_rag.rag.documents import DOCUMENT_SOURCES, ZONING_SOURCES
...
# Schemas are written out the way `urban_rag.postgis` writes them: `rag` for
...
def layer_spec(layer: str) -> TileLayerSpec:
    ...
    try:
        ...
        raise KeyError(
            f"{layer!r} has no tile spec in urban_rag.map_tiles; "
