...
and no retail one at all - see `hbu_dataplatform.marketbeat` - so the retail rent this
...
`hbu_dataplatform.estimator` takes toward the cost guide and the opposite of the one
`hbu_dataplatform.role_foncier` takes toward a published roll year, and the reason is
the same in both: what can be revised is re-read.

Deliberately free of Dagster imports, mirroring `hbu_dataplatform.marketbeat` and
`hbu_dataplatform.estimator`.
"""
...
#: `hbu_dataplatform.comparables` prices floor area under. `total` is carried but
