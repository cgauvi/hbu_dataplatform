...
from hbu_dataplatform.postgis import connect as postgis_connect
from hbu_dataplatform.bdoi import DEFAULT_BASE_URL as BDOI_BASE_URL, BdoiFetcher
...
from hbu_dataplatform.open_data import (
...
from hbu_dataplatform.layers import Layer, layer_of
from hbu_dataplatform.spectrum import DEFAULT_BASE_URL, SpectrumClient
...
class ParquetStore(ConfigurableResource):
    ...
    tile axis (see `hbu_dataplatform.partitions`).
    ...
    The layer is looked up from `hbu_dataplatform.layers` rather than passed in, so a
...
class QuebecZoningResource(ConfigurableResource):
    ...
    the city's map server - see `hbu_dataplatform.quebec`. Same posture as
...
class SaguenayZoningResource(ConfigurableResource):
    ...
    `hbu_dataplatform.saguenay`. The default pause is shorter than the other
...
class AdressesQuebecResource(ConfigurableResource):
    ...
    An ArcGIS MapServer at the MRNF - see `hbu_dataplatform.adresses_quebec`, which
...
class RfuResource(ConfigurableResource):
    ...
    last - see `hbu_dataplatform.rfu.default_rfu_year` for why that is resolved from
...
class RoleResource(ConfigurableResource):
    ...
    disk to be read at all - see `hbu_dataplatform.role_foncier`.
...
class PostgisResource(ConfigurableResource):
    ...
    assets publish through `hbu_dataplatform.warehouse`.
