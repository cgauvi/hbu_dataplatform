...
`hbu_dataplatform.role_foncier` reads `rl0105a` off every assessment unit and this
...
"category of a code" at all. `hbu_dataplatform.comparables.CUBF_CLASSES` is what maps a
...
`hbu_dataplatform.role_foncier` caches 572 MB by filename forever. This is 185 kB at a
...
every scrape date fetches it again. The same posture `hbu_dataplatform.crspi` and
`hbu_dataplatform.estimator` take, and for the same reason.
...
Deliberately free of Dagster imports, mirroring `hbu_dataplatform.open_data`,
`hbu_dataplatform.cmhc` and `hbu_dataplatform.crspi`.
"""
...
#: posture `hbu_dataplatform.cmhc._header_row` takes on the survey workbook.
...
class CubfFetcher:
    ...
    def fetch(self) -> bytes:
        ...
        if not content.startswith(b"PK"):
            ...
            # `hbu_dataplatform.cmhc.CmhcFetcher.fetch` makes.
