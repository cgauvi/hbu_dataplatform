...
EPSG:4326, valid geometry, the crosswalks in `hbu_dataplatform.partitions` applied,
...
*computed* in PostGIS - see `hbu_dataplatform.postgis`.
...
`gold/lot_profiles` is `gold.lot_profiles`. `hbu_dataplatform.warehouse` derives that
...
def layer_of(asset_name: str) -> Layer:
    ...
    try:
        ...
        raise KeyError(
            f"{asset_name!r} has no layer in hbu_dataplatform.layers.ASSET_LAYERS; "
