...
`hbu_dataplatform.zoning_grid` reads it. Quebec City publishes its zones as an ArcGIS
layer and its norms as one workbook for the city, which is what
`hbu_dataplatform.quebec` exists for. Saguenay splits the difference:
...
its own URL, Saguenay goes through `hbu_dataplatform.rag_assets.linked_documents` and
...
#: no Saguenay-specific street feed to name. See `hbu_dataplatform.rqtt`.
...
def _families(codes: Sequence[str]) -> tuple[tuple[str, ...], dict[str, str]]:
    ...
    reason `hbu_dataplatform.quebec.grid_columns` emits them: Saguenay's classes do
