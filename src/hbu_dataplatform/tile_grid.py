...
of what a cell of each layer carries. `hbu_dataplatform.postgis` turns these into the
rollup SQL, `hbu_dataplatform.aggregate_assets` runs it per partition, and
...
#: union. See `hbu_dataplatform.postgis` on why those copies are stored intact rather
...
class LayerSpec:
    ...
    `hbu_dataplatform.postgis` already uses.
...
def _layers() -> dict[str, LayerSpec]:
    ...
    Schemas are written out the way the rest of `hbu_dataplatform.postgis` writes
...
def layer_spec(layer: str) -> LayerSpec:
    ...
    try:
        ...
        raise KeyError(
            f"{layer!r} has no aggregate spec in hbu_dataplatform.tile_grid; "
