...
a stated constant in `hbu_dataplatform.program` until this module, and this is the
...
Deliberately free of Dagster imports, mirroring `hbu_dataplatform.estimator`,
`hbu_dataplatform.role_foncier` and `hbu_dataplatform.cmhc`.
"""
...
#: which is why `hbu_dataplatform.crspi` exists: the retail rent this platform needs
...
class Report:
    ...
    def period_start(self) -> str:
        ...
        `hbu_dataplatform.crspi` publishes a quarter as the month it starts in, so
