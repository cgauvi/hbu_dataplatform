...
#: what bounds the cadastre query - see `hbu_dataplatform.infolot_assets`.
...
#: partition, for `hbu_dataplatform.cmhc_assets`. A third geography keyed the same
...
#: leading `Montréal` taken off - see `hbu_dataplatform.marketbeat`.
#:
...
#: `hbu_dataplatform.rent_assets` says so when it prices one off the Montreal market.
...
#: `hbu_dataplatform.guards` compares a partition key against. If they disagreed, the
...
#: constant (`hbu_dataplatform.tile_cut` says why it is not derived): a registry
