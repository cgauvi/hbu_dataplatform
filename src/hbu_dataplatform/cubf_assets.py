...
It is the companion to `hbu_dataplatform.role_assets.property_assessment_roll` in the
...
job - see `hbu_dataplatform.cubf.use_code_descriptions` - and keeping the headings is
...
from hbu_dataplatform.open_data_assets import GROUP as OPEN_DATA_GROUP
...
#: Shared with `hbu_dataplatform.open_data_assets` and `hbu_dataplatform.role_assets` rather
...
#: Read back by `hbu_dataplatform.role_assets.assessment_units`.
