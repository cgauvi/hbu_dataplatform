...
from hbu_dataplatform.frames import count_invalid_geometries, features_to_frame, write_frame
from hbu_dataplatform.layers import key_prefix
from hbu_dataplatform.open_data import OpenDataError, decode_csv
...
from hbu_dataplatform.storage import clear_parquet, filesystem, join
...
#: `hbu_dataplatform.infolot_assets` to bound each borough's cadastre query.
...
#: on - see `hbu_dataplatform.partitions.NEIGHBORHOOD_BOROUGH_CODES`.
...
#: `hbu_dataplatform.street_assets` to cut each borough's slice out of it.
#:
...
#: fourth city needs a bounding box and no code at all. See `hbu_dataplatform.rqtt`.
...
#: `hbu_dataplatform.partitions.QUEBEC_BOROUGH_ABBREVIATIONS` are. Written beside
...
#: Québec portal Quebec City's layers come from - see `hbu_dataplatform.saguenay` for
...
class StreetNetworkConfig(Config):
    ...
    because the RQTT publishes no municipality code - see `hbu_dataplatform.rqtt`.
...
def borough_boundary(store: ParquetStore, scrape_date: str, neighborhood: str):
    ...
    one borough's shape - see `hbu_dataplatform.infolot_assets` and
    `hbu_dataplatform.bdoi_assets`. A Quebec City key is cut out of the
...
def city_bounds(
    ...
    silver against the real boundary - see `hbu_dataplatform.street_assets`.
    """
