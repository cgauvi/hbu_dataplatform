...
* **Is that a lot of money for this lot?** A cap rate answers it - net
  ...
  is not a dwelling, against `hbu_dataplatform.program`'s stated per-square-foot
  rates.
...
The arithmetic is all in `hbu_dataplatform.comparables`, which has no Dagster imports
...
from hbu_dataplatform.role_foncier import JOIN_KEY
...
def lot_assessment_comparables(
    ...
    # readable by anything. `hbu_dataplatform.warehouse._as_text` writes them straight
...
def _income_assumptions(
    ...
    *,
    ...
    # `hbu_dataplatform.program`'s constants when handed nothing, and those overstate
