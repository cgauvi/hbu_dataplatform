...
own loader in `hbu_dataplatform.postgis`, all of them writing into `rag`, all of them
...
`gold.lot_profiles`. The schema is looked up from `hbu_dataplatform.layers` rather
...
the tile cut (`hbu_dataplatform.tile_cut`), because a cell is ground that cannot be
...
#:   `chunk_id` into a table `hbu_dataplatform.rag.pgvector` creates and hbu_infra's
...
#:   served as a file. See `hbu_dataplatform.tile_assets`.
...
def table_for(dataset: str) -> Table:
    ...
    try:
        ...
        raise KeyError(
            f"{dataset!r} has no table in hbu_dataplatform.warehouse.TABLES; add it "
...
def publish(
    ...
    *,
    ...
    `hbu_dataplatform.resources`, which imports `hbu_dataplatform.postgis`, which imports
    this one.
...
#
# `hbu_dataplatform.digest` decides what a dataset's content hashes to. This is where
...
def _require_key_columns(
    ...
    raise ValueError(
        ...
        + available
        + " Map it in hbu_dataplatform.warehouse.TABLES if the source spells it "
        "differently."
    )
...
def _as_json(value: Any) -> Any:
    ...
    "loosely typed" - numpy scalars, timestamps, and (where `hbu_dataplatform.frames`
