...
which `hbu_dataplatform.bdoi` hands to GDAL through `zip://` without ever touching
...
Deliberately free of Dagster imports, mirroring `hbu_dataplatform.bdoi` and
`hbu_dataplatform.open_data`.
"""
...
#: `hbu_dataplatform.role_assets` joins to Infolot's polygons by lot number rather
...
#: category, which is why `hbu_dataplatform.comparables` reads a character of it and
...
#: `hbu_dataplatform.role_assets.lot_key`, which is what makes the two comparable.
...
#: this pipeline has - see `hbu_dataplatform.partitions`.
...
def default_roll_year() -> int:
    ...
    `hbu_dataplatform.cmhc.default_survey_year`.
    """
...
def read_layer(
    ...
    *,
    ...
    `hbu_dataplatform.bdoi.read_shapefile_zip` does for BDOI's province-wide extracts,
