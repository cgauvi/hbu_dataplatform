...
`hbu_dataplatform.hbu.program` prices a building the way a solver has to - one present
...
**And a building is not all dwellings.** `hbu_dataplatform.hbu.program` fills an envelope
...
* **the exit.** A stabilised income sells at the cap its *own* market pays,
  ...
  the whole reason `hbu_dataplatform.hbu.hbu` carries `hbu_commercial_noi_cad` beside
...
#: same three `hbu_dataplatform.hbu.comparables.INCOME_CLASSES` names, restated so this
...
def blended_cap_rate(
    ...
    **Weighted by income and never by floor.** The whole point of the blend is
    ...
    than the money it actually collects justifies. `hbu_dataplatform.hbu.hbu` writes
