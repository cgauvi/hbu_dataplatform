...
Deliberately free of Dagster imports, the same posture `hbu_dataplatform.sources.roll.client`
and `hbu_dataplatform.hbu.program` take: everything here is arithmetic over frames, and
the asset that calls it is `hbu_dataplatform.hbu.comparables_assets`.
...
by Statistics Canada's rent index. `hbu_dataplatform.hbu.program`'s constants remain the
...
them in `total_assessed_value` - see `hbu_dataplatform.sources.roll.assets` - and this module
...
from both sides of it - see `hbu_dataplatform.hbu.comparables_assets`. `num_candidates`
...
from hbu_dataplatform.sources.roll.client import DWELLINGS_COLUMN as _DWELLINGS
from hbu_dataplatform.sources.roll.client import FLOOR_AREA_COLUMN as _FLOOR_AREA
from hbu_dataplatform.sources.roll.client import LAND_AREA_COLUMN as _LAND_AREA
from hbu_dataplatform.sources.roll.client import (
    NONRESIDENTIAL_UNITS_COLUMN as _NONRESIDENTIAL,
)
from hbu_dataplatform.sources.roll.client import RENTAL_ROOMS_COLUMN as _RENTAL_ROOMS
from hbu_dataplatform.sources.roll.client import STOREYS_COLUMN as _STOREYS
from hbu_dataplatform.sources.roll.client import USE_CODE_COLUMN as _USE_CODE
from hbu_dataplatform.sources.roll.client import VALUE_COLUMN as _VALUE
from hbu_dataplatform.sources.roll.client import YEAR_BUILT_COLUMN as _YEAR_BUILT
from hbu_dataplatform.sources.cubf.client import (
...
#: than over `rl0311a` and `rl0308a`, and `hbu_dataplatform.sources.roll.client` stays the
...
#: The same SRID `hbu_dataplatform.core.postgis` computes frontage and setbacks in, named
...
#: `hbu_dataplatform.sources.cubf.assets` snapshots, so they can be read back rather than
#: taken on trust:
#:
#:     1    RESIDENTIELLE
...
#: `hbu_dataplatform.hbu.hbu` is what acts on it - a parcel the roll calls a road is not a
...
class IncomeAssumptions:
    ...
    `hbu_dataplatform.hbu.program`'s - and ``income_reference_year`` is the year ages are
    ...
    **The age adjustment is off unless ``income_reference_year`` is set.**
    ...
    `hbu_dataplatform.hbu.program`'s constants so a caller that hands over nothing still
    ...
    def residential_occupancy(self) -> float:
        ...
        `hbu_dataplatform.hbu.program` warns about at the top of its own module. A borough
    ...
    def as_metadata(self) -> dict[str, object]:
        ...
        return {
            ...
            # are the curve that ages it. `hbu_dataplatform.hbu.hbu` reads the base back
...
def aggregate_units_by_lot(
    ...
    ``pairs`` is one row per (unit, lot) - `hbu_dataplatform.sources.roll.assets` places them,
    ...
    columns `hbu_dataplatform.sources.roll.client` names. Returns one row per lot, indexed by
    ``lot_column``.
...
def _records(frame: pd.DataFrame) -> list[dict[str, object]]:
    ...
    The same round trip `hbu_dataplatform.hbu.lot_profiles_assets._records` makes, and for
