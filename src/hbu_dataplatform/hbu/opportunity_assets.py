...
on yield on cost, and the arithmetic for both is in `hbu_dataplatform.hbu.opportunities`,
...
which is the one judgement in the formula - see `hbu_dataplatform.hbu.opportunities`.
...
class OpportunityConfig(Config):
    ...
    The rest are the second axis - see `hbu_dataplatform.hbu.opportunities.SiteRules` for
