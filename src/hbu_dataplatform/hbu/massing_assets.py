...
`hbu_dataplatform.hbu.massing`, free of Dagster the way `program`, `comparables` and
...
That split is `hbu_dataplatform.core.warehouse`'s rule rather than this asset's choice - a
...
from hbu_dataplatform.hbu.hbu_assets import LOT_HBU_FILE, lot_highest_best_use
from hbu_dataplatform.core.layers import key_prefix
from hbu_dataplatform.hbu.massing import (
...
class MassingConfig(Config):
    ...
    `hbu_dataplatform.hbu.massing`'s own constants, and the value that produced a table
...
def _massing_only(frame: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    `hbu_dataplatform.core.warehouse` reads a frame's *active* geometry into the table's
