...
`hbu_dataplatform.hbu.program` answers one lot's question and stops there: hand it a
...
Deliberately free of Dagster imports, the posture `hbu_dataplatform.hbu.program`,
`hbu_dataplatform.hbu.comparables` and `hbu_dataplatform.sources.roll.client` take: everything here is
arithmetic over frames, and `hbu_dataplatform.hbu.hbu_assets` is what reads and writes the
partitions.
...
a coincidence - the non-residential rates in both are `hbu_dataplatform.hbu.program`'s,
...
from hbu_dataplatform.hbu.comparables import (
    INCOME_CLASSES,
    IncomeAssumptions,
    is_road_use_code,
)
...
#: `hbu_dataplatform.core.postgis`, because it is the one column of it this module
...
class ProgramAssumptions:
    ...
    The defaults are `hbu_dataplatform.hbu.program`'s own module constants, so a run that
...
def use_gap(
    ...
    **The roll is per lot and the answer is per piece, so the roll is split.**
    ...
    # input rather than a comparison - `hbu_dataplatform.hbu.proforma` divides it by an
    ...
    # these, and `hbu_dataplatform.hbu.proforma` is the reader this exists for.
    #
...
def three_futures(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    every future - and before the site's own costs, which `hbu_dataplatform.
