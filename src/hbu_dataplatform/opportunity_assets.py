...
on yield on cost, and the arithmetic for both is in `hbu_dataplatform.opportunities`,
...
which is the one judgement in the formula - see `hbu_dataplatform.opportunities`.
...
from hbu_dataplatform.proforma import ProformaAssumptions, Timing
...
class OpportunityConfig(Config):
    ...
    The rest are the second axis - see `hbu_dataplatform.opportunities.SiteRules` for
