...
`hbu_dataplatform.warehouse` - see that module for the partitioning and the upsert.
...
(`hbu_dataplatform.tile_cut`) that a lot's representative point falls in - and each
...
`hbu_dataplatform.warehouse` documents the trade in full; the part that matters here
...
from hbu_dataplatform import tile_cut, tile_grid, warehouse
from hbu_dataplatform.rag.pgvector import PgSettings, PostgresUnavailable
from hbu_dataplatform.warehouse import MissingRelation  # noqa: F401  (re-exported)
...
class GroundOutsideCut(RuntimeError):
    ...
    produce nothing. The fix is the one `hbu_dataplatform.tile_cut` describes -
    ...
    def __init__(
        ...
        super().__init__(
            ...
            "hbu_dataplatform.tile_cut before loading this ground."
        )
...
#: every silver and gold write goes through `hbu_dataplatform.warehouse`, which calls
...
def analyze(connection: "Connection", table: str) -> None:
    ...
    `hbu_dataplatform.warehouse._analyze` is the same step for the silver and gold
...
def load_streets(
    ...
    *whole* - a side is never clipped at a cell edge, any more than at a
    ...
    The one loader here that is a plain `hbu_dataplatform.warehouse` call and nothing
    ...
    used to do this with `ST_Multi` in its INSERT; `hbu_dataplatform.warehouse` has no
...
def compute_intersections(
    ...
    *,
    ...
    Published through `hbu_dataplatform.warehouse.upsert_select` rather than inserted
...
#: network became the RQTT (`hbu_dataplatform.rqtt`) the line runs down the axis of the
...
#: (`hbu_dataplatform.rqtt`), which sits half a carriageway further from the lot - five
...
def compute_lot_buildable_setbacks(
    ...
    *Avant secondaire*, *Latérale* and *Arrière* since sql/012 with nothing
    subtracting them, and `hbu_dataplatform.program` caps a footprint on *Taux
...
# where at present it draws nothing at all. `hbu_dataplatform.tile_grid` holds the
...
def _roll_up_level(cursor: Any, zoom: int) -> tuple[int, int]:
    ...
    reason `hbu_dataplatform.tile_grid` gives: the four children of a cell are
...
def compute_map_cell_aggregates(
    ...
    *,
    ...
    to draw instead of nothing. See `hbu_dataplatform.tile_grid` for why a cell is a
...
def load_addresses(
    ...
    *,
    ...
    ``frame`` is the *parsed* bronze snapshot - `hbu_dataplatform.address_assets`
...
def compute_lot_addresses(
    ...
    **The publisher has no lot number**, so this join is the only thing that
    puts an address on a parcel. `hbu_dataplatform.adresses_quebec` says why: the
...
def _as_text(value: Any) -> str | None:
    ...
    `hbu_dataplatform.warehouse._as_text` is the same rule for the silver and gold
...
def _replace_partition(
    ...
    *,
    ...
    `hbu_dataplatform.warehouse` instead, which upserts rather than replaces; these
