...
BDOI extracts in `hbu_dataplatform.bdoi`. It is fetched once and cached on disk keyed
...
`hbu_dataplatform.infolot` and `hbu_dataplatform.bdoi`.
"""
...
def normalize_quartier(name: str) -> str:
    ...
    against `hbu_dataplatform.partitions.CMHC_QUARTIERS`, so the crosswalk holds one
