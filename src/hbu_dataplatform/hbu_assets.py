...
Three assets over one question. `hbu_dataplatform.program` has been able to answer it
...
standing on the ground today. `hbu_dataplatform.hbu` is the arithmetic, free of
...
covering most of the lot across zones. See `hbu_dataplatform.hbu` for why that is not
...
from hbu_dataplatform.partitions import tile_partition_of, tile_scrape_partitions
...
from hbu_dataplatform.warehouse import MissingRelation, publish, published_metadata
...
_GAP_OUTPUT_COLUMNS = (
    ...
    # rate and the lease-up in `hbu_dataplatform.proforma`, above all - wants this.
...
class ProgramConfig(Config):
    ...
    `hbu_dataplatform.program`'s module docstring is careful to say so at each of them.
    ...
    commercial_rent_per_sqft_year_cad: float | None = Field(
        ...
        description=(
            ...
            "off silver/commercial_rents, falling back to hbu_dataplatform.program's "
    ...
    def assumptions(
        ...
        """This run's config as the object `hbu_dataplatform.hbu` is handed.
        ...
        upstream distinguishes. That is `hbu_dataplatform.program`'s own default,
...
@asset(
    ...
    description=(
        ...
        "solver input, each the answer to one hbu_dataplatform.program CP-SAT run. "
...
class GapConfig(Config):
    ...
    `enhance_assumptions`, and `hbu_dataplatform.hbu.EnhancementRules` is what says
...
def _neighborhoods_of_tile(
    ...
    """
    from hbu_dataplatform.postgis import neighborhoods_of_tile
...
def _surveyed_commercial_rents(
    ...
    prices commerce and industry at `hbu_dataplatform.program`'s stated constants,
...
def _with_placeable_area(
    ...
    from hbu_dataplatform.massing import fit_rectangle, to_metric
...
def _with_parkable_area(
    ...
    **The parcel is the piece, and on a split lot the difference is a double
    ...
    """
    from hbu_dataplatform.massing import parking_capacity_m2, to_metric, yard_of
    from hbu_dataplatform.postgis import fetch_lot_polygons, fetch_zone_piece_polygons
