...
is not in the roll - `hbu_dataplatform.comparables` says so where it defaults
...
Deliberately free of Dagster imports, like `hbu_dataplatform.open_data`, so the
...
#: portal from `hbu_dataplatform.open_data.DEFAULT_BASE_URL` (the *city's*) but the
...
#: exists for; `hbu_dataplatform.comparables.MARKET_FACTOR` is the knob it feeds.
...
#: Ville de Montréal. The same code `hbu_dataplatform.role_assets` filters the
...
def default_rfu_year() -> int | None:
    ...
    ``None`` rather than a constant, unlike `hbu_dataplatform.role_foncier.
