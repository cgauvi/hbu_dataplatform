...
from hbu_dataplatform.spectrum import STYLE_COLUMN
from hbu_dataplatform.storage import dirname, filesystem
...
def write_frame(frame: Frame, path: str) -> str:
    ...
    ``path`` is a local path or an ``s3://`` URI - see `hbu_dataplatform.storage`.
