...
with the city it belongs to - so unlike `hbu_dataplatform.neighborhoods` there is
nothing to register. What an operator needs from the axis is two lists::

    python -m hbu_dataplatform.tiles list                    # every cell, by city
    python -m hbu_dataplatform.tiles of VSMPE 2026-09-01     # the cells VSMPE landed in
...
ahead of the runs that write them::
    ...
    python -m hbu_dataplatform.tiles ensure 2026-09-01
...
from hbu_dataplatform import tile_cut
from hbu_dataplatform.partitions import city_of_tile


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="hbu_dataplatform.tiles", description="The cells of the tile cut."
    )
    ...
    from hbu_dataplatform.postgis import connect, tiles_of_neighborhood
...
def _ensure_leaves(scrape_date: str) -> int:
    ...
    from hbu_dataplatform.postgis import connect
    from hbu_dataplatform.rag.pgvector import PgSettings
