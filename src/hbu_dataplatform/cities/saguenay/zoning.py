...
def _reading_order(
    ...
    `zoning_grid.page_rows` sorts on descending ``ty``, which is the top of a
...
def _section_number(row: Sequence[Cell]) -> float | None:
...
def _section_of(row: Sequence[Cell]) -> str | None:
...
def _articles(rows: Sequence[Sequence[Cell]]) -> str | None:
...
def _by_column(
    cells: Iterable[Cell], centers: Sequence[float], reach: float
) -> dict[int, Cell]:
