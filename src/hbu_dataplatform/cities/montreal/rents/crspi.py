...
and no retail one at all - see `hbu_dataplatform.cities.montreal.rents.marketbeat` - so the retail rent this
...
`hbu_dataplatform.cities.montreal.costs.estimator` takes toward the cost guide and the opposite of the one
`hbu_dataplatform.sources.roll.client` takes toward a published roll year, and the reason is
the same in both: what can be revised is re-read.

Deliberately free of Dagster imports, mirroring `hbu_dataplatform.cities.montreal.rents.marketbeat` and
`hbu_dataplatform.cities.montreal.costs.estimator`.
"""
...
#: `hbu_dataplatform.hbu.comparables` prices floor area under. `total` is carried but
