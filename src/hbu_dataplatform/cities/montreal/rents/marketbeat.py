...
a stated constant in `hbu_dataplatform.hbu.program` until this module, and this is the
...
is the same posture `hbu_dataplatform.rag.documents` takes toward the zoning PDFs -
...
Deliberately free of Dagster imports, mirroring `hbu_dataplatform.cities.montreal.costs.estimator`,
`hbu_dataplatform.sources.roll.client` and `hbu_dataplatform.sources.cmhc.client`.
"""
...
#: which is why `hbu_dataplatform.cities.montreal.rents.crspi` exists: the retail rent this platform needs
...
class Report:
    ...
    def period_start(self) -> str:
        ...
        `hbu_dataplatform.cities.montreal.rents.crspi` publishes a quarter as the month it starts in, so
