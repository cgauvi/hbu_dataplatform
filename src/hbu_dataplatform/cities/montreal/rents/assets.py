...
Until these assets that figure was two constants in `hbu_dataplatform.hbu.program` -
...
page because the filenames cannot be constructed. See `hbu_dataplatform.cities.montreal.rents.marketbeat`.
...
`hbu_dataplatform.cities.montreal.rents.crspi`.
...
class RentsConfig(Config):
    ...
    **It is a gross rent**, matching what the office MarketBeat publishes and
    what `hbu_dataplatform.hbu.comparables` charges a tenant - not the net figure a
