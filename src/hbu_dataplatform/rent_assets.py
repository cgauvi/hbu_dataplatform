...
Until these assets that figure was two constants in `hbu_dataplatform.program` -
...
page because the filenames cannot be constructed. See `hbu_dataplatform.marketbeat`.
...
class RentsConfig(Config):
    ...
    **It is a gross rent**, matching what the office MarketBeat publishes and
    what `hbu_dataplatform.comparables` charges a tenant - not the net figure a
