...
def _ask(arguments: argparse.Namespace) -> int:
    ...
    if store is None:
        return 1
    from hbu_dataplatform.rag.chain import build_chain, build_llm
    from hbu_dataplatform.rag.retriever import VectorStoreRetriever
...
def _embeddings(arguments: argparse.Namespace, store: VectorStore | PgVectorStore):
    ...
    """
    from hbu_dataplatform.rag.embeddings import cached_embeddings
