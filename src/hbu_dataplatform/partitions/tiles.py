...
with the city it belongs to - so unlike `hbu_dataplatform.partitions.neighborhoods` there is
nothing to register. What an operator needs from the axis is two lists::

    python -m hbu_dataplatform.partitions.tiles list                    # every cell, by city
    python -m hbu_dataplatform.partitions.tiles of VSMPE 2026-09-01     # the cells VSMPE landed in
...
ahead of the runs that write them::
    ...
    python -m hbu_dataplatform.partitions.tiles ensure 2026-09-01
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="hbu_dataplatform.partitions.tiles", description="The cells of the tile cut."
    )
