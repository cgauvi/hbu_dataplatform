...
The neighborhood axis is dynamic - see `hbu_dataplatform.partitions.axes` - so the set of
boroughs the pipeline scrapes lives in Dagster's instance storage rather than
in code. This is the command that edits it::

    python -m hbu_dataplatform.dagster_home python -m hbu_dataplatform.partitions.neighborhoods list
    python -m hbu_dataplatform.dagster_home python -m hbu_dataplatform.partitions.neighborhoods add CIL
    python -m hbu_dataplatform.dagster_home python -m hbu_dataplatform.partitions.neighborhoods remove CIL
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="hbu_dataplatform.partitions.neighborhoods",
        description=(
