...
from hbu_dataplatform import map_tiles, tile_grid
...
from hbu_dataplatform.hbu_assets import lot_highest_best_use, lot_redevelopment_gap
...
from hbu_dataplatform.partitions import borough_partition_of, scrape_partitions
from hbu_dataplatform.pmtiles_archive import ArchiveSummary, write_archive
...
from hbu_dataplatform.storage import clear_files, join, write_bytes
from hbu_dataplatform.street_assets import neighborhood_streets
from hbu_dataplatform.tile_render import layer_extent, render_layer, tile_fields
...
class MapTilesConfig(Config):
    ...
    the legend says lots. They live in `hbu_dataplatform.map_tiles` as constants.
    """

    layers: list[str] = Field(
        default=list(map_tiles.LAYERS),
        description=(
            "The map layers to render, from hbu_dataplatform.map_tiles.LAYERS. Narrow "
...
def map_tiles_asset(
    ...
    if unknown:
        raise Failure(
            f"{', '.join(unknown)} is not a layer hbu_dataplatform.map_tiles knows how "
...
def _read_manifest(output_dir: str) -> dict[str, Any] | None:
    from hbu_dataplatform.storage import read_text  # noqa: PLC0415
