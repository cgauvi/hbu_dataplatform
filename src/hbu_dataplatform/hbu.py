...
`hbu_dataplatform.program` answers one lot's question and stops there: hand it a
...
Deliberately free of Dagster imports, the posture `hbu_dataplatform.program`,
`hbu_dataplatform.comparables` and `hbu_dataplatform.role_foncier` take: everything here is
arithmetic over frames, and `hbu_dataplatform.hbu_assets` is what reads and writes the
partitions.
...
a coincidence - the non-residential rates in both are `hbu_dataplatform.program`'s,
...
#: `hbu_dataplatform.postgis`, because it is the one column of it this module
...
class ProgramAssumptions:
    ...
    The defaults are `hbu_dataplatform.program`'s own module constants, so a run that
...
def use_gap(
    ...
    **The roll is per lot and the answer is per piece, so the roll is split.**
    ...
    # input rather than a comparison - `hbu_dataplatform.proforma` divides it by an
    ...
    # these, and `hbu_dataplatform.proforma` is the reader this exists for.
    #
