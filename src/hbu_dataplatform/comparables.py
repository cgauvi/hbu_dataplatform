...
Deliberately free of Dagster imports, the same posture `hbu_dataplatform.role_foncier`
and `hbu_dataplatform.program` take: everything here is arithmetic over frames, and
the asset that calls it is `hbu_dataplatform.comparables_assets`.
...
by Statistics Canada's rent index. `hbu_dataplatform.program`'s constants remain the
...
them in `total_assessed_value` - see `hbu_dataplatform.role_assets` - and this module
...
from both sides of it - see `hbu_dataplatform.comparables_assets`. `num_candidates`
...
#: than over `rl0311a` and `rl0308a`, and `hbu_dataplatform.role_foncier` stays the
...
#: The same SRID `hbu_dataplatform.postgis` computes frontage and setbacks in, named
...
#: `hbu_dataplatform.cubf_assets` snapshots, so they can be read back rather than
#: taken on trust:
#:
#:     1    RESIDENTIELLE
...
#: `hbu_dataplatform.hbu` is what acts on it - a parcel the roll calls a road is not a
...
class IncomeAssumptions:
    ...
    `hbu_dataplatform.program`'s - and ``income_reference_year`` is the year ages are
    ...
    **The age adjustment is off unless ``income_reference_year`` is set.**
    ...
    `hbu_dataplatform.program`'s constants so a caller that hands over nothing still
    ...
    def residential_occupancy(self) -> float:
        ...
        `hbu_dataplatform.program` warns about at the top of its own module. A borough
    ...
    def as_metadata(self) -> dict[str, object]:
        ...
        return {
            ...
            # are the curve that ages it. `hbu_dataplatform.hbu` reads the base back
...
def aggregate_units_by_lot(
    ...
    ``pairs`` is one row per (unit, lot) - `hbu_dataplatform.role_assets` places them,
    ...
    columns `hbu_dataplatform.role_foncier` names. Returns one row per lot, indexed by
    ``lot_column``.
...
def _records(frame: pd.DataFrame) -> list[dict[str, object]]:
    ...
    The same round trip `hbu_dataplatform.lot_profiles_assets._records` makes, and for
