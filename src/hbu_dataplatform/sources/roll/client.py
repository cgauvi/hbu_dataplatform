...
which `hbu_dataplatform.sources.bdoi.client` hands to GDAL through `zip://` without ever touching
...
Deliberately free of Dagster imports, mirroring `hbu_dataplatform.sources.bdoi.client` and
`hbu_dataplatform.core.open_data`.
"""
...
#: `hbu_dataplatform.sources.roll.assets` joins to Infolot's polygons by lot number rather
...
#: category, which is why `hbu_dataplatform.hbu.comparables` reads a character of it and
...
#: `hbu_dataplatform.sources.roll.assets.lot_key`, which is what makes the two comparable.
...
#: this pipeline has - see `hbu_dataplatform.partitions.axes`.
...
def default_roll_year() -> int:
    ...
    `hbu_dataplatform.sources.cmhc.client.default_survey_year`.
    """
...
def read_layer(
    ...
    *,
    ...
    `hbu_dataplatform.sources.bdoi.client.read_shapefile_zip` does for BDOI's province-wide extracts,
