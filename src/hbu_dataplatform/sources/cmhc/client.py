...
BDOI extracts in `hbu_dataplatform.sources.bdoi.client`. It is fetched once and cached on disk keyed
...
`hbu_dataplatform.sources.infolot.client` and `hbu_dataplatform.sources.bdoi.client`.
"""
...
def normalize_quartier(name: str) -> str:
    ...
    against `hbu_dataplatform.partitions.axes.CMHC_QUARTIERS`, so the crosswalk holds one
