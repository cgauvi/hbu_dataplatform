...
`hbu_dataplatform.sources.roll.client` reads `rl0105a` off every assessment unit and this
...
"category of a code" at all. `hbu_dataplatform.hbu.comparables.CUBF_CLASSES` is what maps a
...
`hbu_dataplatform.sources.roll.client` caches 572 MB by filename forever. This is 185 kB at a
...
every scrape date fetches it again. The same posture `hbu_dataplatform.cities.montreal.rents.crspi` and
`hbu_dataplatform.cities.montreal.costs.estimator` take, and for the same reason.
...
Deliberately free of Dagster imports, mirroring `hbu_dataplatform.core.open_data`,
`hbu_dataplatform.sources.cmhc.client` and `hbu_dataplatform.cities.montreal.rents.crspi`.
"""
...
#: posture `hbu_dataplatform.sources.cmhc.client._header_row` takes on the survey workbook.
...
class CubfFetcher:
    ...
    def fetch(self) -> bytes:
        ...
        if not content.startswith(b"PK"):
            ...
            # `hbu_dataplatform.sources.cmhc.client.CmhcFetcher.fetch` makes.
