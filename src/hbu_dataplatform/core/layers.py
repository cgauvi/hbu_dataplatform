...
EPSG:4326, valid geometry, the crosswalks in `hbu_dataplatform.partitions.axes` applied,
...
*computed* in PostGIS - see `hbu_dataplatform.core.postgis`.
...
`gold/lot_profiles` is `gold.lot_profiles`. `hbu_dataplatform.core.warehouse` derives that
...
ASSET_LAYERS: dict[str, Layer] = {
    ...
    "neighborhood_addresses": Layer.BRONZE,
    "linked_documents": Layer.BRONZE,
    "council_minutes": Layer.BRONZE,
    "council_minutes_documents": Layer.BRONZE,
...
def layer_of(asset_name: str) -> Layer:
    ...
    try:
        ...
        raise KeyError(
            f"{asset_name!r} has no layer in hbu_dataplatform.core.layers.ASSET_LAYERS; "
