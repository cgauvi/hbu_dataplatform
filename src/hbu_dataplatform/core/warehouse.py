...
own loader in `hbu_dataplatform.core.postgis`, all of them writing into `rag`, all of them
...
`gold.lot_profiles`. The schema is looked up from `hbu_dataplatform.core.layers` rather
...
the tile cut (`hbu_dataplatform.core.tile_cut`), because a cell is ground that cannot be
...
#:   served as a file. See `hbu_dataplatform.map.tile_assets`.
...
def table_for(dataset: str) -> Table:
    ...
    try:
        ...
        raise KeyError(
            f"{dataset!r} has no table in hbu_dataplatform.core.warehouse.TABLES; add it "
...
def publish(
    ...
    *,
    ...
    `hbu_dataplatform.core.resources`, which imports `hbu_dataplatform.core.postgis`, which imports
    this one.
...
def _require_key_columns(
    ...
    raise ValueError(
        ...
        + available
        + " Map it in hbu_dataplatform.core.warehouse.TABLES if the source spells it "
        "differently."
    )
...
def _as_json(value: Any) -> Any:
    ...
    "loosely typed" - numpy scalars, timestamps, and (where `hbu_dataplatform.core.frames`
