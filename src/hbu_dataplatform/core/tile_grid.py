...
of what a cell of each layer carries. `hbu_dataplatform.core.postgis` turns these into the
rollup SQL, `hbu_dataplatform.map.aggregate_assets` runs it per partition, and
...
#: union. See `hbu_dataplatform.core.postgis` on why those copies are stored intact rather
...
class LayerSpec:
    ...
    `hbu_dataplatform.core.postgis` already uses.
...
#: in `hbu_dataplatform.core.postgis` already binds.
_PARTITION = (
...
def _layers() -> dict[str, LayerSpec]:
    ...
    Schemas are written out the way the rest of `hbu_dataplatform.core.postgis` writes
...
def layer_spec(layer: str) -> LayerSpec:
    ...
    try:
        ...
        raise KeyError(
            f"{layer!r} has no aggregate spec in hbu_dataplatform.core.tile_grid; "
