...
#: plain value of `hbu_dataplatform.partitions.axes.City` (a string here rather than the
...
def cell_partition_of(cell_key: str, cut: frozenset[str] | None = None) -> str:
    ...
    if not live:
        raise RuntimeError(
            "the tile cut has not been seeded: hbu_dataplatform.core.tile_cut.CUT is "
