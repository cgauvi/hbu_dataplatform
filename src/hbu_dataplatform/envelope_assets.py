...
`hbu_dataplatform.program` answers one question - what unit mix is the envelope of
...
`hbu_dataplatform.zoning_grid` for why that is a question about x-coordinates. A grid
...
`hbu_dataplatform.zone_piece_assets`.
...
from hbu_dataplatform.quebec import GRID_SLUG, GRID_ZONE_COLUMN, ZONING_SLUG, grid_columns
...
from hbu_dataplatform.saguenay import parse_grid_pdf as parse_saguenay_grid_pdf
...
@asset(
    ...
    description=(
        ...
        "columns - see hbu_dataplatform.quebec.grid_columns for what is translated. "
...
def zoning_grid_columns(
    ...
    if city_of(neighborhood) is City.QUEBEC:
        ...
        # rest of this branch is shared. See `hbu_dataplatform.saguenay`.
        parse = (
...
@asset(
    ...
    description=(
        ...
        "hbu_dataplatform.program reads: one row per (lot, zone, grid column), "
...
def _governing(envelopes: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    *sites*, each with its own area and its own street, so each picks its own
    ...
    other piece's width. See `hbu_dataplatform.zone_piece_assets`.
