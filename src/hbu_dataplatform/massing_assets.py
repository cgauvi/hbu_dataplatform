...
`hbu_dataplatform.massing`, free of Dagster the way `program`, `comparables` and
...
That split is `hbu_dataplatform.warehouse`'s rule rather than this asset's choice - a
...
from hbu_dataplatform.massing import (
...
class MassingConfig(Config):
    ...
    `hbu_dataplatform.massing`'s own constants, and the value that produced a table
...
def _massing_only(frame: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    `hbu_dataplatform.warehouse` reads a frame's *active* geometry into the table's
