...
Free of Dagster and of psycopg the way `tile_grid` is. `hbu_dataplatform.map.tile_render`
turns these specs into the ``ST_AsMVT`` statements that produce the tiles,
`hbu_dataplatform.map.pmtiles_archive` packs a layer's tiles into one PMTiles file, and
`hbu_dataplatform.map.tile_assets` runs both per partition and writes the result into the
...
# Schemas are written out the way `hbu_dataplatform.core.postgis` writes them: `rag` for
...
def layer_spec(layer: str) -> TileLayerSpec:
    ...
    try:
        ...
        raise KeyError(
            f"{layer!r} has no tile spec in hbu_dataplatform.map.map_tiles; "
