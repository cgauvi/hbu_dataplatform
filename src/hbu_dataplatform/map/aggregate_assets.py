...
The grid arithmetic and the per-layer measures are in `hbu_dataplatform.core.tile_grid`;
the rollup SQL is `hbu_dataplatform.core.postgis.compute_map_cell_aggregates`; the table is
hbu_infra's sql/023.
...
from hbu_dataplatform.hbu.hbu_assets import lot_redevelopment_gap
...
class MapAggregateConfig(Config):
    ...
    layers: list[str] = Field(
        ...
        description=(
            "The map layers to dissolve, from hbu_dataplatform.core.tile_grid.LAYERS. "
...
def map_cell_aggregates(
    ...
    if unknown:
        raise Failure(
            f"{', '.join(unknown)} is not a layer hbu_dataplatform.core.tile_grid knows "
    ...
    # The tree keeps a copy for the reason `hbu_dataplatform.core.layers` gives for every
