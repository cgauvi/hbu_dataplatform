...
| [development-program.md](development-program.md) | `urban_rag.program` and `silver.lot_development_programs` — the CP-SAT model behind every answer: its decision variables, the printed norms and the three shape caps that bind them, the three places a stall can go, the discounted-net-profit objective, and the `binding` vocabulary that says why an answer is not bigger |
