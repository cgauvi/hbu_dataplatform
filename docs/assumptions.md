...
| a parcel is the roadway when ≥ 1 m of RQTT street line runs inside it; a ruelle is not a street | `FrontageConfig.min_street_m` | config | the RQTT publishes no `Ruelle` class and draws only 50 named laneways in the Montreal box, so a lot backing onto a lane gets no frontage from it. This is coverage rather than a rule, so it is worth re-checking each vintage — [street-frontage.md](street-frontage.md) |
