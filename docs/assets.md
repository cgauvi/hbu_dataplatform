...
| silver | `lot_zoning_envelopes` | date × tile | Every zoning envelope in the borough, denormalised to the grain `urban_rag.program` reads — one row per (lot, zone, grid column), with the **piece** that zone governs (`piece_area_m2`), the parcel it belongs to (`lot_area_m2`), that piece's own primary and secondary frontage, and the norms that bound what may be built on it — as parquet and as `silver.lot_zoning_envelopes` |
