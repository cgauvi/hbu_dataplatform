...
All 41 assets, their partition axes and what each one writes. The layer
...
| silver | `neighborhood_streets` | date × neighborhood | That day's RQTT centre lines clipped to one borough — a segment crossing the borough line is `ST_Intersection`-ed against the boundary — with the published length, the length inside it, and the share that survived the cut — as geoparquet and as `silver.neighborhood_streets` |
| silver | `lot_frontage` | date × neighborhood | How much of each lot's boundary faces each street, in metres, longest first — the boundary it shares with the street's own cadastral parcel, so an exact edge with no buffer. As geoparquet and as `silver.lot_frontage`, with the parcels that *are* the street beside it as `road_lots.parquet` — the set nothing else on the platform holds, since the roll never reached Montreal's roadways, and what keeps them out of the highest-and-best-use inventory. **Blocked**, see below |
