...
| silver | `lot_zoning_envelopes` | date × tile | Every zoning envelope in the borough, denormalised to the grain `hbu_dataplatform.hbu.program` reads — one row per (lot, zone, grid column), with the **piece** that zone governs (`piece_area_m2`), the parcel it belongs to (`lot_area_m2`), that piece's own primary and secondary frontage, and the norms that bound what may be built on it — as parquet and as `silver.lot_zoning_envelopes` |
...
| silver | `lot_development_programs` | date × tile | One `hbu_dataplatform.hbu.program.solve_program` CP-SAT run per candidate row of `lot_zoning_envelopes` that authorises dwellings, commerce or industry and parses — the mix of dwellings, commerce, industry and parking that maximises discounted net profit (`npv_cad`) under that envelope, with the storey split, footprint, stalls, build cost, the legacy monthly NOI and `binding` caps — as parquet and as `silver.lot_development_programs`. **Blocked**, see below |
...
they were first parsed with nothing subtracting them. `hbu_dataplatform.hbu.program` caps
