...
# The scrape partition is monthly (see hbu_dataplatform.partitions.axes), so the key is
...
# under an earlier month's key. They refuse it; see hbu_dataplatform.partitions.guards and
# docs/running.md.
DATE ?= $(shell date +%Y-%m-01)
# A key from hbu_dataplatform.partitions.axes.known_neighborhoods(): a Montreal borough
...
# a stable link. See hbu_dataplatform.core.postgis.DEFAULT_SETBACK_BATCH_LOTS.
SETBACK_BATCH ?= 2000
SETBACK_RESUME ?= true
...
# hbu_dataplatform.core.postgis.DEFAULT_ADDRESS_SNAP_M.
ADDRESS_SNAP_M ?= 2.0
...
# (hbu_dataplatform.partitions.axes.MUNICIPALITY_CODES); `[]` keeps the province. See
...
# what a by-law demands - hbu_dataplatform.hbu.program's own default. 0 removes the
...
# see hbu_dataplatform.hbu.hbu_assets.ProgramConfig.
RES_COST_SQFT ?= 257.5
...
# assumptions - see hbu_dataplatform.hbu.program.InvestmentAssumptions.
DISCOUNT_PCT ?= 5.0
HOLD_YEARS ?= 25
...
# The map layers `make map_tiles` renders - hbu_dataplatform.map.map_tiles.LAYERS, all
...
# refuses a key hbu_dataplatform.partitions.axes cannot resolve into its sources.
neighborhoods: | $(UV_SYNC_STAMP) ## List the boroughs registered on the partition axis, and the ones that could be
	$(URBAN_RAG_PYTHON) -m hbu_dataplatform.partitions.neighborhoods list

neighborhood-add: | $(UV_SYNC_STAMP) ## Register NEIGHBORHOOD on the partition axis (e.g. NEIGHBORHOOD=CIL)
	$(URBAN_RAG_PYTHON) -m hbu_dataplatform.partitions.neighborhoods add $(NEIGHBORHOOD)

neighborhood-remove: | $(UV_SYNC_STAMP) ## Take NEIGHBORHOOD off the axis; its partitions stay on disk
	$(URBAN_RAG_PYTHON) -m hbu_dataplatform.partitions.neighborhoods remove $(NEIGHBORHOOD)
...
# The tile axis is static - every cell of hbu_dataplatform.core.tile_cut.CUT, checked in
...
tiles: | $(UV_SYNC_STAMP) ## List the cells of the tile cut, by city
	$(URBAN_RAG_PYTHON) -m hbu_dataplatform.partitions.tiles list

tiles-of: | $(UV_SYNC_STAMP) ## The cells NEIGHBORHOOD's loaded lots fall in for DATE (needs the database)
	$(URBAN_RAG_PYTHON) -m hbu_dataplatform.partitions.tiles of $(NEIGHBORHOOD) $(DATE)
...
# other one behind the first. See hbu_dataplatform.partitions.tiles.
tiles-ensure: | $(UV_SYNC_STAMP) ## Create every tile table's partition for every cell of DATE
	$(URBAN_RAG_PYTHON) -m hbu_dataplatform.partitions.tiles ensure $(DATE)
...
# been indexed - see hbu_dataplatform.hbu.lot_profiles_assets. Also reads three silver
...
# hbu_dataplatform.hbu.program is run once per candidate row it wrote. `setbacks` is
...
# it at hbu_dataplatform.hbu.program's stated constants, which flatter retail badly.
...
# side; see hbu_dataplatform.hbu.hbu_assets.ProgramConfig for the rest.
